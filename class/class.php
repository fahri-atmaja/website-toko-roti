<?php  
	...
	class DataBase{
		...
		private $user = "u1549220_tokohjkarja";
		private $pass = "tokohjkarja";
		private $db = "u1549220_tokohjkarja";
	...
	class Admin{
		...
		public function simpan_admin($email,$pass,$nama,$status,$gambar){
			...
			//insert
			mysql_query("INSERT INTO admin(email,password,nama,gambar,status) VALUES('$email','$pass','$nama','$namafile','$status')");
		}
		...
		public function ubah_admin($email,$pass,$nama,$gambar,$id,$status){
			$namafile = $gambar['name'];
			$lokasifile = $gambar['tmp_name'];
			...
			if (!empty($lokasifile)) {
				...
				mysql_query("UPDATE admin 
					SET email = '$email', password='$pass', nama='$nama', gambar='$namafile', status='$status' WHERE kd_admin='$id'");
			}
			else{
				//update tanpa upload gambar
				mysql_query("UPDATE admin 
					SET email = '$email', password='$pass', nama='$nama', status='$status' WHERE kd_admin='$id'");
			}
		}
		...
		public function login_admin($email,$pass){
			...
			if ($cocokan > 0) {
				...
				$_SESSION['login_admin']['status'] = $data['status'];
	...
	class EOQ{
	    public function tampil_eoq(){
			$qry = mysql_query("SELECT * FROM eoq ORDER BY nama_barang ASC");
			while ($pecah = mysql_fetch_array($qry)) {
				$data[] = $pecah;
			}
			return $data;
		}
		public function hitung_eoq($nmbrg,$periode){
		    $cekdemand = mysql_query("SELECT SUM(jumlah) as demand FROM d_penjualan
	                                        LEFT JOIN barang ON barang.kd_barang=d_penjualan.kd_barang
	                                        LEFT JOIN penjualan ON penjualan.kd_penjualan=d_penjualan.kd_penjualan
	                                        WHERE barang.nama_barang='$nmbrg' AND (penjualan.tgl_penjualan BETWEEN '$periode-01' AND '$periode-31')");
	        $arr = mysql_fetch_assoc($cekdemand);
	        return $arr;
		}
		public function chart($nmbrg){
		    $cekdemand = mysql_query("SELECT SUM(jumlah) as demand FROM d_penjualan
	                                        LEFT JOIN barang ON barang.kd_barang=d_penjualan.kd_barang
	                                        LEFT JOIN penjualan ON penjualan.kd_penjualan=d_penjualan.kd_penjualan
	                                        WHERE barang.nama_barang='$nmbrg'");
	        $arr = mysql_fetch_assoc($cekdemand);
	        return $arr;
		}
		public function harga_barang($nmbrg){
		    $cekdemand = mysql_query("SELECT harga_jual FROM barang WHERE nama_barang='$nmbrg'");
	        $arr = mysql_fetch_assoc($cekdemand);
	        return $arr;
		}
	    public function simpan_eoq($kode_eoq,$nama_barang,$periode,$biaya_administrasi,$lead_time){
			mysql_query("INSERT INTO eoq(kode_eoq,nama_barang,periode,biaya_administrasi,lead_time) 
				VALUES('$kode_eoq','$nama_barang','$periode','$biaya_administrasi','$lead_time')");
		}
		public function ubah_eoq($nama_barang,$periode,$biaya_administrasi,$lead_time,$kode_eoq){
			mysql_query("UPDATE eoq SET nama_barang='$nama_barang', periode='$periode', biaya_administrasi='$biaya_administrasi', lead_time='$lead_time' WHERE kode_eoq = '$kode_eoq'");
		}
		public function ambil_eoq($id){
			$qry = mysql_query("SELECT * FROM eoq WHERE kode_eoq = '$id'");
			$pecah = mysql_fetch_assoc($qry);

			return $pecah;
		}
		public function hapus_eoq($id){
			mysql_query("DELETE FROM eoq WHERE kode_eoq = '$id'");
		}
	}
	class Barang{
	    public function kode_barang($nmbrg){
			$qry = mysql_query("SELECT * FROM barang WHERE nama_barang = '$nmbrg'");
			while ($pecah = mysql_fetch_array($qry)) {
				$data[] = $pecah;
			}
			return $data;
		...
		public function simpan_stok_barang($kdbarang,$kdbl,$stok){
// 			$dat = $this->ambil_barangpem($kdbl);
// 			$stok = $dat['item'];
...
// 			mysql_query("UPDATE barang_pembelian SET status='1' WHERE kd_barang_beli ='$kdbl'");
	...
	class Pembelian{
		...
		public function email_pembelian($kdpembelian,$tglpembelian,$supplier,$totalpem){
		    $mail = new PHPMailer;
            $mail->IsSMTP();
            $mail->SMTPSecure = ‘tls’;
                ...
                <div class="container">
                    <div class="row">
                        ...
                        <table border="0px" padding="1" align="center" style="width: 600px;" >
                            <tr>
                                <td>Email</td><td>:</td><td>1111</td>
                            </tr>
                            <tr>
                                <td>Password</td><td>:</td><td>1111</td>
                            </tr>
                        </table>
                        <h3 align="center">Panduan PMB UNDARIS</h3>
                        <h4 align="center">
                        Silahkan login menggunakan email dan kode login diatas. klik disini <a href="https://pmb.undaris.ac.id/calonmahasiswa/login.php">https://pmb.undaris.ac.id/calonmahasiswa/login.php</a><br>
                        <h3 align="center">
...
// 			$email = email_stok($kdbarang);
		}
		public function email_stok($kdbarang) {
	...
	class Perusahaan{
		...
		public function simpan_perusahaan($nama,$alamat,$pemilik,$kota,$email){
			mysql_query("UPDATE perusahaan SET nama_perusahaan='$nama',alamat='$alamat', pemilik='$pemilik', kota='$kota', email='$email' WHERE kd_perusahaan ='1' ");
		}
	}
	class Log{
		public function tampil_log(){
			$qry = mysql_query("SELECT log.*, admin.nama FROM log
			                    LEFT JOIN admin ON admin.kd_admin=log.kd_admin");
			while ($pecah = mysql_fetch_array($qry)) {
				$data[] = $pecah;
			}
			return $data;
		}
		public function simpan_log($kd_admin,$deskripsi,$waktu){
			mysql_query("INSERT INTO log(kd_admin,deskripsi,waktu) 
				VALUES('$kd_admin','$deskripsi','$waktu')");
		}
	}
	class Dashboard{
